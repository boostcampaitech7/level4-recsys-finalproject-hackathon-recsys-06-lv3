...
def load_model_and_mappings(model_path, config):
    ...
    return model, (user2user_encoded, user_encoded2user, anime2anime_encoded, anime_encoded2anime), df

def get_top_k_recommendations(model, user_id, encoded_user_id, anime_encoded2anime, k=10):
    ...
    rating_tensor = torch.full((num_items,), 0.5, dtype=torch.float).to(device)  # 중간값으로 설정
...
def validate_user_ids(user_ids, df):
    ...
    if invalid_users:
        print(f"Warning: Following user IDs were not found in the dataset: {invalid_users}")
...
def main():
    ...
    model, (user2user_encoded, user_encoded2user, anime2anime_encoded, anime_encoded2anime), df = \

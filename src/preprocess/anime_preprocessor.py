...
class AnimePreProcessor(AbstractPreProcessor):
    def __init__(self, dataset: str, data_path, export_path, sample_size: int = 10):
        ...
        self.data = {
            "anime_with_synopsis": pd.read_csv(
                os.path.join(data_path, "anime_with_synopsis.csv")
            ),
    ...
    def _filter_users_with_interactions(self, ratings) -> pd.DataFrame:
        ...
        user_interaction_counts = filtered_ratings[
            filtered_ratings["interaction"] == 1
        ]["user_id"].value_counts()
        filtered_users_with_interactions = user_interaction_counts[
            user_interaction_counts >= 11
        ].index
    ...
    def _split_train_test(self, ratings) -> tuple:
        test = (
            ...
            .groupby("user_id")
            .apply(lambda x: x.sample(n=self.sample_size, random_state=42))
